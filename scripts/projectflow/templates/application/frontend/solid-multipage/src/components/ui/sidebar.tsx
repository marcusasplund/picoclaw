    ...
    <Switch>
      ...
      <Match when={isMobile()}>
        ...
        <Sheet open={openMobile()} onOpenChange={setOpenMobile} {...others}>
          <SheetContent
            ...
            data-mobile="true"
            class="w-(--sidebar-width) border-sidebar-border !bg-sidebar p-0 text-sidebar-foreground shadow-xl"
            style={{
          ...
          >
            ...
            <SheetDescription class="sr-only">Choose a page.</SheetDescription>
            <div
              class="flex size-full flex-col !bg-sidebar"
              style={{ "background-color": "hsl(var(--sidebar-background))" }}
              onTouchStart={(event) => {
              ...
              onTouchEnd={(event) => {
                ...
                if (local.side === "left" && deltaX <= -60) {
                  setOpenMobile(false)
                }

                if (local.side === "right" && deltaX >= 60) {
                  setOpenMobile(false)
                }
              }}
            >
      ...
      <Match when={!isMobile()}>
        ...
        >
          ...
          <div
            class={cn(
              "w-(--sidebar-width) fixed inset-y-0 z-10 hidden h-svh transition-transform duration-200 ease-out motion-reduce:transition-none md:flex",
              local.side === "left"
...
const SidebarTrigger = <T extends ValidComponent = "button">(props: SidebarTriggerProps<T>) => {
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      class={cn("h-7 w-auto px-2 md:size-7 md:px-0", local.class)}
    ...
    >
      ...
      <span aria-hidden="true" class="text-sm md:hidden">Menu</span>
      <span class="sr-only">Toggle Sidebar</span>
    </Button>
  )
}

const SidebarRail: Component<ComponentProps<typeof Button>> = (props) => {
...
const SidebarGroupLabel = <T extends ValidComponent = "div">(
  ...
  return (
    <Polymorphic<SidebarGroupLabelProps>
      ...
      class={cn(
        "flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium text-sidebar-foreground/70 outline-none ring-sidebar-ring transition-opacity duration-200 ease-out focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
...
const SidebarMenuButton = <T extends ValidComponent = "button">(
  ...
  const props = mergeProps({ isActive: false, size: "default", variant: "default" }, rawProps)
  ...
  const button = (
    <Polymorphic<SidebarMenuButtonProps>
      ...
      class={cn(
        sidebarMenuButtonVariants({ size: local.size, variant: local.variant }),
        local.class
      )}
      {...others}
    />
