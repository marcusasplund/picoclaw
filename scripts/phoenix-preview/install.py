...
IMAGE = 'sha256:8f166856fa585edd1e0d21b627848466f880889e4bb824f30e59b2bfac92ab76'
HOST = 'preview-phoenix.marcusasplund.com'
PROJECT = 'picoclaw-phoenix-preview'
ROOT = Path('/opt/picoclaw-phoenix-preview')
PORT = 4187
AUTH = '/etc/nginx/picoclaw-preview.htpasswd'
AVAILABLE = Path('/etc/nginx/sites-available/picoclaw-phoenix-preview')
ENABLED = Path('/etc/nginx/sites-enabled/picoclaw-phoenix-preview')
MARKER = '# Owned by picoclaw-phoenix-preview\n'
...
def spec(dbimage):
    ...
    migration.update({'profiles': ['migration'], 'restart': 'no',
                      'command': ['bin/du_chat', 'eval', 'DuChat.Release.migrate()']})
    app.update({'restart': 'unless-stopped', 'ports': [f'127.0.0.1:{PORT}:4000'],
    ...
    return {'name': PROJECT, 'services': {'app': app, 'migrate': migration,
            ...
            'volumes': {'data': {}}, 'networks': {'private': {'internal': True}}}
...
def main():
    ...
    with open('/var/log/picoclaw-phoenix-preview.log', 'ab') as log:
        ...
        def poll(check):
            for _ in range(30):
                try:
                    ...
                    pass
                time.sleep(2)
            raise RuntimeError('Health check timed out; inspect /var/log/picoclaw-phoenix-preview.log')
        ...
        (ROOT / 'receipt.json').write_text(json.dumps({'status': 'passed', 'image_id': IMAGE,
            'url': 'https://' + HOST + '/api/', 'project': PROJECT, 'platform': platform}, indent=2) + '\n')
        announce('Klart: https://' + HOST + '/api/ — använd befintlig preview-inloggning')
