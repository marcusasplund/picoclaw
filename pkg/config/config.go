...
func (c *TelegramSettings) SetToken(token string) {
...
type SlackSettings struct {
	StaticFlowCommand []string     `json:"static_flow_command,omitempty" yaml:"-"`
...
func applyChannelEnvCompat(cfg *Config) {
	if cfg == nil {
		return
	}
	for name, channel := range cfg.Channels {
		...
		envKey := "PICOCLAW_CHANNELS_" + strings.ToUpper(strings.ReplaceAll(channelType, "-", "_")) + "_ENABLED"
		envKey = strings.ReplaceAll(envKey, "__", "_")
		if raw, ok := os.LookupEnv(envKey); ok {
			if enabled, err := strconv.ParseBool(strings.TrimSpace(raw)); err == nil {
				channel.Enabled = enabled
			}
		}
		commonPrefix := strings.TrimSuffix(envKey, "_ENABLED")
		if raw, ok := os.LookupEnv(commonPrefix + "_ALLOW_FROM"); ok {
			var allowFrom FlexibleStringSlice
			if err := allowFrom.UnmarshalText([]byte(raw)); err == nil {
				channel.AllowFrom = allowFrom
			}
		}
		if raw, ok := os.LookupEnv(commonPrefix + "_REASONING_CHANNEL_ID"); ok {
			channel.ReasoningChannelID = strings.TrimSpace(raw)
		}
	}
}
