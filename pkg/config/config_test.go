...
func TestLoadConfig_SMSGatewayURLFromJSON(t *testing.T) {
	dir := t.TempDir()
	configPath := filepath.Join(dir, "config.json")
	raw := `{"version":2,"channels":{"sms":{
	...
	sms := cfg.Channels.Get(ChannelSMS)
	if sms == nil {
		t.Fatal("sms channel missing")
	}
	decoded, err := sms.GetDecoded()
	if err != nil {
		t.Fatalf("sms.GetDecoded() error: %v", err)
	}
	smsCfg, ok := decoded.(*SMSConfig)
	if !ok {
		t.Fatalf("sms decoded config = %T, want *SMSConfig", decoded)
	}

	if !sms.Enabled {
		t.Fatal("sms.enabled = false, want true")
	}
	if smsCfg.GatewayURL != "http://127.0.0.1:8080" {
		t.Fatalf("gateway_url = %q, want %q", smsCfg.GatewayURL, "http://127.0.0.1:8080")
	}
	if smsCfg.APIKey != "secret" {
		t.Fatalf("api_key = %q, want %q", smsCfg.APIKey, "secret")
	}
	if smsCfg.PollInterval != 7 {
		t.Fatalf("poll_interval = %d, want 7", smsCfg.PollInterval)
	}
	if smsCfg.TimeoutSeconds != 11 {
		t.Fatalf("timeout_seconds = %d, want 11", smsCfg.TimeoutSeconds)
	}
	if !smsCfg.DeleteAfterRead {
