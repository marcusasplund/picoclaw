...
- [ ] Support user-directed visual experiments during a run: the latest creative brief overrides template styling and aesthetic preferences. For example, "make it 8-bit looking" may change typography, palette, borders, icons, motion, components, and layout while preserving required behavior and accessibility. Persist the revised direction and intentional departures in the brief and `DESIGN.md`, apply existing plan/version approval rules, and rerun affected checks.
...
- [ ] Make review policy distinguish measured failures, heuristic findings, and design judgment. Evaluate experiments against the user's chosen direction; aesthetic heuristics must not independently block release or undo intentional styling. Keep functional and accessibility checks required. Verify with a brief that intentionally departs from the starter (such as 8-bit styling), including a real accessibility defect that must still be repaired.
